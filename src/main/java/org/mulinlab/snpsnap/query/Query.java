...
import org.mulinlab.snpsnap.index.Index;
import org.mulinlab.snpsnap.index.IndexWriter;
import org.mulinlab.snpsnap.index.MAF;
...
public final class Query {
    ...
    public Query(final String input) throws IOException {
        this.input = input;
    ...
    public void loadMAF(final String chr, final int maf) throws IOException {
        List<MAF> mafList = index.getMafList(chr);
        int flag;

        if(lastMAF != null) {
            flag = isInRangeMAF(maf, lastMAF.getMaf());
            if(flag == LESS) {
                if(linkMap.get(maf) != null) linkMap.remove(maf);
                if(tempMap.get(maf) != null) tempMap.remove(maf);
            } else if(flag == GREATER) {
                return;
            } else {
                loadDB(lastMAF.getAddress(), lastMAF.getMaf());
            }
        }

        for(int k = 0; k < mafList.size(); k++) {
            flag = isInRangeMAF(maf, mafList.get(k).getMaf());
            if(flag == LESS) {
                if (linkMap.get(mafList.get(k).getMaf()) != null) linkMap.remove(mafList.get(k).getMaf());
                if (tempMap.get(mafList.get(k).getMaf()) != null) tempMap.remove(mafList.get(k).getMaf());
                mafList.remove(k);
                k--;
            } else{
                break;
        ...
        for (MAF dbmaf:mafList) {
            flag = isInRangeMAF(maf, dbmaf.getMaf());

            if(flag == GREATER) {
                lastMAF = dbmaf;
                break;
            } else {
                loadDB(dbmaf.getAddress(), dbmaf.getMaf());
            }
        }
    }
    ...
    public int isInRangeMAF(final Integer nodeMaf, final Integer dbmaf) {
        if(dbmaf < nodeMaf + mafRange[0]) {
            return LESS;
        } else if(dbmaf > nodeMaf + mafRange[1]) {
            return GREATER;
        } else {
            return IN;
        }
    ...
    public int isInRangeDistance(final Integer nodeDistance, final Integer dbDistance) {
        if(dbDistance < nodeDistance + disRange[0]) {
            return LESS;
        } else if(dbDistance > nodeDistance + disRange[1]) {
            return GREATER;
        } else {
            return IN;
        }
